# Contributors

VibeCheck was built for Eclipse 6.0 (ACM Thapar Student Chapter, April 2026) by **Team Pipped Pipers**.

## Core Engineering

**Arun AK** ([@Arun07AK](https://github.com/Arun07AK))
- Sole engineer on the technical product
- Built end-to-end: backend, all 4 static scanners (secrets, dependencies, PII, code smells), Playwright MCP integration, the Claude Sonnet 4.6 agentic loop, the 5 specialized AI agents, the SQLite knowledge base, the scoring engine, and the entire React frontend
- Architected the Model Context Protocol bridge connecting Claude to a real headless browser via 21 structured tool calls

## Team Contributions

**Dev Ariwala** — Ideation, UI polish, pitch presenter
- Co-ideated the product concept
- Contributed to UI design refinements
- Presented the project to judges in the pitch round

**Yash Jag** — Test app builder
- Built the vibe-coded test applications used as scan targets during demos
- Created `broken-todo-app` and other intentionally flawed apps that VibeCheck scans

**Adaa** — UI polish & design
- Refined visual design elements
- Contributed to deck design

**Abhijay** — Pitch planning
- Strategized the pitch presentation
- Planned the demo flow

---

## Recognition

Pipped Pipers finished **top 27 of 258 teams** at Eclipse 6.0.

## License

This project is licensed under the MIT License — see [LICENSE](LICENSE) for details.
Copyright (c) 2026 Arun AK.
