...
const BADGE_RULES = [
  ...
  { id: 'dependency-hell', name: 'Dep Hell', Icon: Flame, description: 'Vulnerable dependencies', check: (d) => d.scannerCounts.dependencies > 0 },
  ...
  { id: 'xss-artist', name: 'XSS Risk', Icon: Drama, description: 'innerHTML unsanitized', check: (d) => d.issues.some(i => i.title.includes('innerHTML') || i.title.includes('document.write')) },
  { id: 'clean-room', name: 'Clean Room', Icon: Sparkles, description: 'Zero critical/high', check: (d) => d.severityCounts.CRITICAL === 0 && d.severityCounts.HIGH === 0 },
  { id: 'fortress', name: 'Fort Knox', Icon: Shield, description: 'No secrets leaked', check: (d) => d.scannerCounts.secrets === 0 },
  { id: 'perfect-score', name: 'Flawless', Icon: Gem, description: 'Score 95+', check: (d) => d.score >= 95 },
];
...
function Badges({ data }) {
  ...
  return (
    <div className="space-y-2">
      <h3 className="text-[10px] font-semibold text-dim uppercase tracking-widest">Badges</h3>
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2">
        {earned.map((badge) => (
          <div key={badge.id} className="bg-surface border border-border rounded-lg p-3 text-center hover:border-cyan/30 transition-colors">
            <badge.Icon className="w-4 h-4 mx-auto mb-1.5 text-cyan" strokeWidth={1.5} />
            <div className="text-[10px] font-semibold text-white">{badge.name}</div>
            <div className="text-[9px] text-dim mt-0.5">{badge.description}</div>
          </div>
        ))}
      </div>
    </div>
